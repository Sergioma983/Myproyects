  ...
  for (var i = 0; i < nombresHojasOrigen.length; i++) {
    ...
    while (archivos.hasNext()) {
      ...
      if (hojaOrigen) {
        ...
        var areasPorProcesar = [['DEVOLUCIONES', 'COSTOS'], ['FINANZAS', 'MENSAJERIA'],['CONTABILIDAD', 'NOMINA', 'FACTURACIÓN PLANTA']]; // Grupos de áreas
...
function crearHojaAreasUnicas(archivoOrigen, tablaAreas, areasPorProcesar) {
  ...
  for (var i = 0; i < areasPorProcesar.length; i++) {
    ...
    hojaCiertasAreas.getRange(2, 1, ciertasAreasData.length, ciertasAreasData[0].length).setValues(ciertasAreasData);
  }
}
