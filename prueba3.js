...
function aplicarTransformacionesEnCarpeta() {
  var carpetaId = '1Lm4rlmBY57vmSXQj4fcLb0INpy9E0svh';
  ...
  for (var i = 0; i < nombresHojasOrigen.length; i++) {
    ...
    while (archivos.hasNext()) {
      ...
      if (hojaOrigen) {
        ...
        var areasPorProcesar = [['DEVOLUCIONES', 'COSTOS'], ['FINANZAS', 'MENSAJERIA'],['CONTABILIDAD', 'NOMINA', 'FACTURACIÓN PLANTA']];
        if(hojaOrigen.getName() == 'Desgloce Administracion' || hojaOrigen.getName() == 'Desgloce Produccion (Compras)'){
            transformarTablaPorGrupos(archivoOrigen, tablaDatos, areasPorProcesar);}
          else {
...
function transformarTablaPorGrupos(archivoOrigen, tablaDatos, areasPorProcesar) {
  ...
  crearHojaAreasUnicas(archivoOrigen, tablaAreas, areasPorProcesar);
...
function transformarTabla(archivoOrigen, tablaDatos) {
  ...
  eliminarPestanasDesgloce(archivoOrigen);
  crearHojaAreasUnicas2(archivoOrigen, tablaAreas);
  crearPestanasDesgloce(archivoOrigen, tablaAreas, desglosePorArea);
}
...
function crearHojaAreasUnicas(archivoOrigen, tablaAreas, areasPorProcesar) {
  var hojaNueva = archivoOrigen.getSheetByName('Áreas Únicas');
  
  if (!hojaNueva) {
    hojaNueva = archivoOrigen.insertSheet('Áreas Únicas');
  }
  var areasUnicasData = tablaAreas.filter(row => !areasPorProcesar.flat().includes(row[1]));
  hojaNueva.getRange(2, 1, areasUnicasData.length, areasUnicasData[0].length).setValues(areasUnicasData);

  for (var i = 0; i < areasPorProcesar.length; i++) {
    var areaGroup = areasPorProcesar[i];
    var hojaCiertasAreas = archivoOrigen.insertSheet('Ciertas Áreas ' + (i + 1));

    var ciertasAreasData = tablaAreas.filter(row => areaGroup.includes(row[1]));
    hojaCiertasAreas.getRange(1, 1, 1, tablaAreas[0].length).setValues([tablaAreas[0]]);
    hojaCiertasAreas.getRange(1, 1, ciertasAreasData.length, ciertasAreasData[0].length).setValues(ciertasAreasData);
  }
}

function crearHojaAreasUnicas2(archivoOrigen, tablaAreas) {
